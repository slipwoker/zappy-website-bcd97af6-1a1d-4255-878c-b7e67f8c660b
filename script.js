        ...
        (async function() {
          try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: 'bcd97af6-1a1d-4255-878c-b7e67f8c660b',
                name: data.name || '',
                email: data.email || '',
            ...
            if (response.ok) {
              ...
              console.log('⚠️ Zappy: Server returned error status', response.status, result);
            }
          } catch (error) {
            console.error('❌ Zappy: Failed to send email', error);
            // Don't break existing functionality - form will still show success message
          }
        })();
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "גליה באהבה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
